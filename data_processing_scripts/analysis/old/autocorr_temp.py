...
- Outputs under:
  ...
  with filenames prefixed by SUFFIX_TAG:
    ...
    * agg_den_{linear,semilog,loglog}.png
...
def _iter_num_den_rows(df: pd.DataFrame, which: str) -> Iterable[Tuple[np.ndarray, np.ndarray]]:
...
# -------------------- Sanity check for denominator arrays --------------------

def _sanity_den(den: np.ndarray, tag: str = ""):
    """
    Quick sanity checks for denominator arrays (den_all / den_bnd).

    Flags:
      - Non-monotonic segments (should be non-decreasing)
      - Internal zeros / gaps (should be contiguous from r=0..r_max)
      - NaNs or negative entries
    """
    if den.size == 0:
        return
    bad = False
    ...
    if not np.all(np.isfinite(den)) or np.any(den < 0):
        print(f"[WARN] NaN/negative denom values in {tag}")
        bad = True
    ...
    if np.any(diffs < -1e-9):
        bad = True
        idx = np.where(diffs < 0)[0][:5]
        print(f"[WARN] Non-monotone denom in {tag} at indices {idx.tolist()}")

    # Contiguity of nonzeros
    nz = np.flatnonzero(den > 0)
    if nz.size:
        if nz[-1] - nz[0] + 1 != nz.size:
            bad = True
            gaps = np.where(np.diff(nz) > 1)[0][:5]
            if gaps.size:
                print(f"[WARN] Gaps in denom radii in {tag}; "
                      f"example gaps near {nz[gaps]}→{nz[gaps+1]}")
    if (not bad) and (np.random.rand() < 0.001):
        print(f"[OK] denom check passed ({tag}) len={len(den)}")
    ...
    for run_tag in run_tags:
        ...
        for pq in shard_paths:
            ...
            for num_arr, den_arr in _iter_num_den_rows(df, WHICH):
                _sanity_den(den_arr, run_tag)                 # <--- sanity check
...
def _collect_sp_aggregates() -> Tuple[np.ndarray, np.ndarray, Dict[str, int]]:
    ...
    for pq in _discover_sp_parquets():
        ...
        for num_arr, den_arr in _iter_num_den_rows(df, WHICH):
            _sanity_den(den_arr, pq)                          # <--- sanity check
            agg_num = _dynamic_add(agg_num, num_arr)
            agg_den = _dynamic_add(agg_den, den_arr)
            counts["kept_rows"] += 1
...
def main():
    ...
    out_den_lin   = f"{base}__agg_den_linear.png"
    out_den_semi  = f"{base}__agg_den_semilog.png"
    out_den_log   = f"{base}__agg_den_loglog.png"
    ...
    # Plots — denominator
    _plot_series(r_index, agg_den,   "linear",  "aggregated denominator", out_den_lin)
    _plot_series(r_index, agg_den,   "semilog", "aggregated denominator", out_den_semi)
    _plot_series(r_index, agg_den,   "loglog",  "aggregated denominator", out_den_log)
    ...
    metrics = {
        ...
        "outputs": {
            "plots": [
                out_num_lin, out_num_semi, out_num_log,
                out_den_lin, out_den_semi, out_den_log,
                out_rat_lin, out_rat_semi, out_rat_log
            ],
