#!/usr/bin/env python3
"""
validate_sp_autocorr_bd_outputs.py

QA for per-cloud Parquet shards + metadata produced by:
  sp_autocorr_bd_from_model.py  (single p)
    ...
    for part in parts:
        ...
        for rg in range(pf.num_row_groups):
            ...
            for i in range(n):
                ...
                p_val = _val(p_c, i)
                if p_val is None:
                    print(f"[ERR] row {gid}: missing p_val")
                    errs += 1
                if (meta_p is not None) and (p_val is not None):
                    if not np.isclose(float(p_val), meta_p, rtol=1e-10, atol=1e-12):
                        print(f"[ERR] row {gid}: p_val={p_val} != meta p={meta_p}")
                        errs += 1

                # --- boundary histogram arrays
                bd_r = _val(bd_r_c, i)
                bd_counts = _val(bd_counts_c, i)
