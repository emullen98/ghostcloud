...
def fit(xdata, ydata, func=lambda x, p0, p1: p0 * x + p1, xmin=None, xmax=None, sigma=None, ci=0.95, test='rsq',
    ...
    if test == 'rsq' or test == 'adjusted_rsq':
        ...
        if pval <= 1 - ci:
            message = "Chi squared test suggests model does not fit data with p = %.2e less than pcrit = %.2e" % (
            pval, 1 - ci)
        else:
            message = "Chi squared test cannot reject null hypothesis that data are drawn from the same distribution with p = %.2e greater than pcrit = %.2e" % (
            pval, 1 - ci)
    ...
    #see https://arxiv.org/pdf/1012.3754.pdf
        ...
        adjusted_chi_squared = np.sum((y - yguess) ** 2 / ((len(y) - len(
            p1))))  #using adjusted chi squared. Should be not too different from 1 for a reasonable fit. But, chi squared can be easily messed up by a scaling factor.
