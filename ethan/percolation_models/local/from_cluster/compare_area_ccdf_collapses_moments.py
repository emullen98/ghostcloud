...
Compare area collapses of SP & DP lattices across probabilities using moments.
Uses the CCDF files.
"""
import os
import numpy as np
...
ax1[0].set_xlabel(f'$A \\cdot \\langle A^{moment_for_rescaling} \\rangle ^{{-1 / (5 - \\kappa_{{\\text{{area}}}})}}$', fontsize=20)
ax1[0].set_ylabel(f'$C(A) \\cdot \\langle A^{moment_for_rescaling} \\rangle ^{{-(1 - \\kappa_{{\\text{{area}}}}) / ({moment_for_rescaling + 1} - \\kappa_{{\\text{{area}}}})}}$', fontsize=20)
...
ax1[1].set_xlabel(f'$A \\cdot \\langle A^{moment_for_rescaling} \\rangle ^{{-1 / (5 - \\kappa_{{\\text{{area}}}})}}$', fontsize=20)
ax1[1].set_ylabel(f'$C(A) \\cdot \\langle A^{moment_for_rescaling} \\rangle ^{{-(1 - \\kappa_{{\\text{{area}}}}) / ({moment_for_rescaling + 1} - \\kappa_{{\\text{{area}}}})}}$', fontsize=20)
ax1[1].set_title('$\\textbf{Above} ~ p_c$')
fig1.suptitle(f'SP vs. DP area CCDF collapse \n System size = 50000, $\\kappa_{{\\text{{area}}}} = {kappa_area}$')
fig1.savefig(f'./area_ccdf_collapse_compare_moments.pdf')
